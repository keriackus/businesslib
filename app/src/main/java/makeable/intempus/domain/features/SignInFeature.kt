...
class SignInFeature(
    completionBlock: (error: Throwable?, objects: ArrayList<Object>?) -> Void,
    var username: String,
    var password: String
) : Feature(completionBlock) {
    override fun prepareInitialBusinessActions() {
        addBusinessAction(BaseAuthUsecase(this, username, password))
        addBusinessAction(ApiKeyAuthUsecase(this, username, password))
    }
}